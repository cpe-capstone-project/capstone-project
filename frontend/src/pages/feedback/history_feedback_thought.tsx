...
export default function FeedbackHistoryThought() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [feedbacks, setFeedbacks] = useState<FeedBackInterface[]>([]);
    const [thought, setthought] = useState<ThoughtRecordInterface | null>(null);
    ...
    useEffect(() => {
        if (id) {
            setLoading(true);
            // ดึง Feedbacks
            GetFeedbackByThoughtID(Number(id))
                .then((data) => setFeedbacks(data))
                .catch((err) => console.error(err));

            // ดึงข้อมูล Diary
            GetThoughtRecordById(Number(id))
                .then((data) => setthought(data.data))
                .catch((err) => console.error(err))
