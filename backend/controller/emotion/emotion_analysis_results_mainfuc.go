...
const HUGGING_FACE_API_KEY = "APIKEY"
...
func AnalyzeThoughtRecordEmotionsOnly(c *gin.Context) {
	...
	// Check if ThoughtRecord already has EmotionsID (not 0)
	var thoughtRecord entity.ThoughtRecord
	db := config.DB()
	result := db.First(&thoughtRecord, emotionAnalysis.ThoughtRecordID)
	if result.Error != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to get thought record: " + result.Error.Error(),
		})
		return
	}

	if thoughtRecord.EmotionsID != 0 {
		c.JSON(http.StatusConflict, gin.H{
			"error": "มีอารมณอยู่แล้ว",
		})
		return
	}
	...
	// Step 6: Update ThoughtRecord's EmotionsID based on primary_emotion
	err = updateThoughtRecordEmotionsID(emotionAnalysis.ThoughtRecordID, emotionResults.PrimaryEmotion)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update thought record emotions ID: " + err.Error()})
		return
	}
